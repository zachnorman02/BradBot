...
class Migration010(Migration):
    ...
    def up(self):
        # Create sequence for polls table
        try:
            db.execute_query("""
                CREATE SEQUENCE IF NOT EXISTS main.polls_id_seq
            """, fetch=False)
            print(f"   ✅ Created polls_id_seq sequence")
        except Exception as e:
            if "already exists" in str(e).lower():
                print(f"   ℹ️  Polls sequence already exists")
            else:
                raise
        ...
        db.execute_query("""
            CREATE TABLE IF NOT EXISTS main.polls (
                id INTEGER PRIMARY KEY DEFAULT nextval('main.polls_id_seq'),
                guild_id BIGINT NOT NULL,
        ...
        # Create sequence for poll_responses table
        try:
            db.execute_query("""
                CREATE SEQUENCE IF NOT EXISTS main.poll_responses_id_seq
            """, fetch=False)
            print(f"   ✅ Created poll_responses_id_seq sequence")
        except Exception as e:
            if "already exists" in str(e).lower():
                print(f"   ℹ️  Poll responses sequence already exists")
            else:
                raise
        ...
        db.execute_query("""
            CREATE TABLE IF NOT EXISTS main.poll_responses (
                id INTEGER PRIMARY KEY DEFAULT nextval('main.poll_responses_id_seq'),
